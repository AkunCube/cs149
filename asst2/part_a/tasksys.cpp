  ...
  while (true) {
    ...
    int cur_task = 0;
    {
      ...
      work_cv.wait(lock, [this]() {
        // Wake up when shutdown or new task is available.
        return shutdown ||
               (current_runnable && next_task < cur_num_total_tasks);
      });
      if (shutdown) {
        break;
      }
    ...
    cur_r->runTask(cur_task, cur_total);
    int completed = ++num_completed;
    if (completed == cur_total) {
      done_cv.notify_one();
    }
  }
}
